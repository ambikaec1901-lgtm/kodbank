...
const NAV_ITEMS = [
    ...
    { icon: '🤖', label: 'Chat with AI', key: 'ai', highlight: true },
]
        ...
        <div className="db-shell">
                ...
                <nav className="db-nav">
                    {NAV_ITEMS.map(item => (
                        <button
                            key={item.key}
                            id={`nav-${item.key}`}
                            className={`db-nav-item ${activeNav === item.key ? 'active' : ''} ${item.highlight ? 'db-nav-ai' : ''}`}
                            onClick={() => { setActiveNav(item.key); setShowProfile(false) }}
                        >
                            <span className="db-nav-icon">{item.icon}</span>
                            <span>{item.label}</span>
                            {item.highlight && <span className="db-nav-ai-badge">NEW</span>}
                        </button>
                    ))}
                </nav>
            </aside>
            ...
            <div className="db-main">
                ...
                <div className="db-content">
                    ...
                    {activeNav === 'ai' && <ChatAIView profile={profile} />}
                </div>
            </div>
